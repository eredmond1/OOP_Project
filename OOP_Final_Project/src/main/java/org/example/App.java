...
public class App extends Application {
    ...
    private HBox createTextInput() {
        ...
        addBtn.setOnAction(event -> {
            ...
            // draw a line between the new point and the previous point
            if (locations.size() > 1) {
                Location firstLocation = locations.get(locations.size() - 2);
                Location secondLocation = locations.get(locations.size() - 1);
                addLine(firstLocation.getLatitude(), firstLocation.getLongitude(),
                        secondLocation.getLatitude(), secondLocation.getLongitude(),
                        locationGraphics, "#00FF00", SimpleLineSymbol.Style.SOLID, 2.0f);
            }
        });
    ...
    private void createMap() {
        ...
        mapView.setOnMouseClicked(event -> {
            ...
            Point wgsPoint = (Point) GeometryEngine.project(
            ...
            if (locations.size() > 1) {
                ...
                addLine(firstLocation.getLatitude(), firstLocation.getLongitude(),
                        secondLocation.getLatitude(), secondLocation.getLongitude(),
                        locationGraphics, "#0000FF", SimpleLineSymbol.Style.SOLID, 2.0f);
            }
    ...
    private static void addLine(double lat1, double lon1, double lat2, double lon2,
                                GraphicsOverlay graphicsOverlay, String colour,
                                SimpleLineSymbol.Style style, float width) {

        // Create a polyline from two points using WGS84 coordinates
        PointCollection points = new PointCollection(SpatialReferences.getWgs84());
        points.add(lon1, lat1);
        points.add(lon2, lat2);
        Polyline lineGeometry = new Polyline(points);

        // Create a line symbol with the specified style, color, and width
        SimpleLineSymbol lineSymbol = new SimpleLineSymbol(style, Color.valueOf(colour), width);

        // Create a graphic with the line geometry and symbol
